		...
		case WM_COMMAND:
			if (HIWORD(wParam) == CBN_SELCHANGE)
			{
				...
				else
				{
					...
					if (totalSpace < 127)
					{
						ErrorHandling(hwnd, L"USB drive is smaller than minimum requirement!");
						return TRUE;
					}

					else if (freeSpace < totalSpace)
					{
						int msgboxID_LS = MessageBoxW(hwnd, L"The space of your USB drive is less than actual space. It is required to reformat it to FAT32. Continue?", L"Warning", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2);
						switch (msgboxID_LS)
						{
						case IDYES:
				...
				else
				{
					...
					{
						int msgboxID_IC = MessageBoxW(hwnd, L"The filesystem of your USB drive is incompatible to the software. It is required to reformat it to FAT32. Continue?", L"Warning", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2);
